...
const TheatreScreen = () => {
  ...
  const subscribe = async() => {
    const response = await fetch("https://backend-shoes-app.onrender.com/payments/intents", {
      method: "POST",
      body: JSON.stringify({
    ...
    if (!response.ok) return Alert.alert(data.message,"hii");
    ...
    if (presentSheet.error) return Alert.alert(presentSheet.error.message,"jii");
      ...
      >
        <View style={{ flexDirection: "row", alignItems: "center" }}>
          ...
          <View style={{ marginLeft: 6 }}>
            <Text style={{ fontSize: 1, fontWeight: "400" }}>
              {route.params.name}
            </Text>
            <Text
              style={{
